package com.astdev.jpandas.src.main.java.com.c_bata;
...
public class DataFrame<K, S> extends HashMap<K, S> {
    public HashMap<String, Double> mean () {
        ...
        for (K key : this.keySet()) {
            meanDataFrame.put((String) key, ((Series<?>) Objects.requireNonNull(this.get(key))).mean());
        }
    ...
    public HashMap<String, Double> std () {
        HashMap<String, Double> meanDataFrame = new HashMap<String, Double>();
        for (K key : this.keySet()) {
            meanDataFrame.put((String) key, ((Series<?>) Objects.requireNonNull(this.get(key))).std());
        }
        return meanDataFrame;
    }
